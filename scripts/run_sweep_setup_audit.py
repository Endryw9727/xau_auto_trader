"""Audit the Asia liquidity-sweep + reclaim setup family (read-only).

Builds the sweep setup across all instruments, both sides, over a small grid of
reward:risk targets, then runs the full honest stack on the WHOLE family at once
(walk-forward + Bonferroni/Benjamini-Hochberg + Deflated-Sharpe / PBO). Testing
the whole R:R grid together — rather than cherry-picking the best R:R — keeps the
multiple-testing / DSR bars honest.
...
from src.analysis.edge_overfitting import align_return_matrix
...
from src.analysis.sweep_setup import asia_sweep_trades
...
DEFAULT_DETAIL_CSV = "sweep_setup_detail.csv"
DEFAULT_SUMMARY_CSV = "sweep_setup_summary.csv"
DEFAULT_LATEST_TXT = "sweep_setup_latest.txt"
DEFAULT_RR_GRID = (1.5, 2.0, 3.0)
DEFAULT_COST_R = 0.02
...
def _build_family(config: dict, rr_grid, cost_r: float) -> dict[str, pd.Series]:
    ...
    for instrument in config.get("instruments", []):
        ...
        try:
            ...
            continue
        for rr in rr_grid:
            trades = asia_sweep_trades(data, symbol, reward_risk=rr, cost_r=cost_r)
            for key, series in trades.items():
                family[f"{key}/RR{rr}"] = series
    return family


def run_sweep_setup_audit(
    *,
    config_path: str | Path = DEFAULT_CONFIG_PATH,
    output_dir: str | Path = DEFAULT_OUTPUT_DIR,
    rr_grid=DEFAULT_RR_GRID,
    ...
    family = _build_family(config, rr_grid, cost_r)
    if len(family) < 2:
    ...
    summary = {
        ...
        "expected_max_sharpe_under_null": of_report.get("expected_max_sharpe_under_null"),
        "live_armed": False,
    }
    _write_reports(paths, detail, summary)
    return SweepAuditResult("OK", "OK", paths["detail"], paths["summary"], paths["latest"])


def _write_reports(paths: dict[str, Path], detail: pd.DataFrame, summary: dict) -> None:
    detail.sort_values("mean_net_pct", ascending=False).to_csv(paths["detail"], index=False)
...
def _build_latest_text(detail: pd.DataFrame, summary: dict) -> str:
    ...
    top = detail.sort_values("mean_net_pct", ascending=False).head(8) if "mean_net_pct" in detail else detail.iloc[0:0]
    lines = [
        "Asia Liquidity-Sweep + Reclaim Setup Audit",
        "=" * 72,
        f"Status: {summary.get('status')}",
        f"Family size (instruments x sides x R:R grid): {summary.get('family_size')}",
        f"Walk-forward robust: {summary.get('walk_forward_robust')}",
        f"Multiple-testing survivors (mtc_robust): {summary.get('mtc_survivors')}",
        f"Expected max Sharpe under null (luck): {summary.get('expected_max_sharpe_under_null')}",
        ...
        "",
        "Most positive (in-sample mean R per trade):",
    ]
    for _, row in top.iterrows():
        lines.append(f"  {row['strategy']:<34} n={row['trades']:>5} meanR={row['mean_net_pct']:>+7} oos_t={row['oos_t_stat']:>6}")
    lines.append("")
    if not survivors.empty:
        lines.append(f"Survivors: {len(survivors)} strategy(ies) survive walk-forward + multiple-testing.")
    else:
        lines.append("Survivors: NONE — the sweep setup is promising in-sample but not yet robust.")
    lines += [
        "",
        "The whole R:R grid is tested together so the DSR / multiple-testing bars",
        "account for the search. A survivor here would still need Monte Carlo and",
        "live-safe checks. Research only. No orders were sent.",
        "",
    ]
    return "\n".join(lines) + "\n"
...
def _write_empty(paths: dict[str, Path], status: str, reason: str) -> SweepAuditResult:
    ...
    paths["latest"].write_text(
        "\n".join(
            ["Asia Liquidity-Sweep + Reclaim Setup Audit", "=" * 72, f"Status: {status}",
             f"Reason: {reason}", "No orders were sent. This is diagnostics only.", ""]
        ),
        encoding="utf-8",
    )
    return SweepAuditResult(status, reason, paths["detail"], paths["summary"], paths["latest"])
...
def main() -> None:
    args = parse_args()
    result = run_sweep_setup_audit(config_path=args.config, output_dir=args.output_dir, n_splits=args.n_splits)
    print("=" * 72)
    print("XAU Auto Trader - Asia Sweep + Reclaim Setup Audit")
    print("=" * 72)
